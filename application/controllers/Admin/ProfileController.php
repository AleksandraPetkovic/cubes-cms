...
class Admin_ProfileController extends Zend_Controller_Action {
    ...
    public function editAction() {
        ...
        $user = Zend_Auth::getInstance()->getIdentity(); //dobija se iz sesije
        ...
        if ($request->isPost() && $request->getPost('task') === 'save') {
            ...
            try {
                ...
                if (!$form->isValid($request->getPost())) {
                    throw new Application_Model_Exception_InvalidInput('Invalid data has been sent for profile');
                }
                ...
                //redirect to same or another page tj admin_dashboard to je nasa stranica
                $redirector = $this->getHelper('Redirector');
                $redirector->setExit(true)
                        ->gotoRoute(array(
                            'controller' => 'admin_profile',
...
//        $form->populate($user);
        ...
        if ($request->isPost() && $request->getPost('task') === 'change_password') {
            ...
            try {
                ...
                //get form data// cuva podatke iz forme $formData
                $formData = $form->getValues();
